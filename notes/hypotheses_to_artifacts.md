...
| H2 (operator consistency) | $\sup_{s\in R}\|\Lambda_N(s)-\Lambda(s)\|\to 0$ | **Formally analytic** (operator theory + truncation argument; not proven numerically). Mechanism note: `notes/lemma_truncation_to_dnmap_convergence.md`. **Numeric proxy / evidence**: convergence sweep `E_Qlambda_convergence_sweep.csv` (per $N$, per basis/block) tracks interior-grid stability of the extracted scalar channel via `err_phi_norm_*` and fit trends in `E_Qlambda_convergence_fit.csv`. This is evidence of *observable* stabilization, not an operator-norm certificate. |
...
| H4 (divisor-clean boundary) | $\phi_{mod}$ has no zeros/poles on $\partial R$ and the quotient is nonvanishing on $\partial R$ (winding=0) | Analytic discharge for “divisor-free boundaries exist”: `notes/lemma_modular_divisor_free_boundary.md`. Operational hygiene for a chosen rectangle: `E_Qlambda_convergence_winding.csv` (`winding`, `failures`, `n_edge`, plus `phi_abs_min` and `phi_bad` to audit that sampled $|\phi_{mod}|$ stays away from 0 and that $\phi$ evaluation succeeded on $\partial R$). Also: divisor diagnostics `E_divisor_arg_principle_*.csv` (non-convergence runs) with recorded `n_edge`/rectangle bounds. |
