...
def main():
    ...
    ap.add_argument("--out_root", default="out_phase3B_preregistered_uniqueness_64_127_v10_phasefieldsolve")
    ...
    # v10 knobs
    ap.add_argument("--v10_phasefield_graph", default="path")
    ap.add_argument("--v10_phasefield_w", type=float, default=1.0)
    ap.add_argument("--v10_phasefield_eta", type=float, default=0.25)
    ...
    # Preregistered backend set (2 only): legacy + v10-autotune
    backends = [
        {"backend_label": "legacy", "tp_backend": "legacy"},
        {
            "backend_label": "geom_v10_phasefieldsolve_autotune",
            "tp_backend": "geom_warp_dirac_v10_phasefieldsolve",
            "geom_v10_phasefield_graph": str(args.v10_phasefield_graph),
            "geom_v10_phasefield_w": float(args.v10_phasefield_w),
            "geom_v10_phasefield_eta": float(args.v10_phasefield_eta),
            "eps_auto_tune": True,
    ...
    for b in backends:
        ...
        for seed in seeds:
            for aseed in anchors:
                ...
                # Backend-specific knobs (v10)
                if "geom_v10_phasefield_graph" in b:
                    cfg["geom_v10_phasefield_graph"] = str(b["geom_v10_phasefield_graph"])
                if "geom_v10_phasefield_w" in b:
                    cfg["geom_v10_phasefield_w"] = float(b["geom_v10_phasefield_w"])
                if "geom_v10_phasefield_eta" in b:
                    cfg["geom_v10_phasefield_eta"] = float(b["geom_v10_phasefield_eta"])
    ...
    pcol = "p_family" if "p_family" in big.columns else ("p_dnmap" if "p_dnmap" in big.columns else None)
    if len(big) and "reject" in big.columns and pcol is not None:
        by_seed = (
            ...
            .agg(
                ...
                anchor_min_p=(pcol, "min"),
                anchor_median_p=(pcol, "median"),
                median_dn_identity_rank=("dn_identity_rank", "median") if "dn_identity_rank" in big.columns else (pcol, lambda x: np.nan),
                rows=("reject", "size"),
            )
            .reset_index()
        )
    ...
    uniq_cols = [
        ...
        "tp_comm_max",
        "geom_v10_eps_global",
        "geom_v10_eps_global_auto",
        "geom_v10_phasefield_graph",
        "geom_v10_phasefield_w",
        "geom_v10_phasefield_eta",
        "v10_theta_sol_digest",
        "eps_auto_tune",
        "eps_target_delta",
    ]
    ...
    print(f"DONE prereg v10-scan: rows={len(big)} elapsed={time.time()-t0:.1f}s out_root={out_root}")
