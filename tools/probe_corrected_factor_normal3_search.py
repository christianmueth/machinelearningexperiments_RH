...
def _objective(eigs: np.ndarray, *, A1: float, A2: float, A3: float) -> float:
    ...
    err2 = abs(B2 - target2)
    err3 = abs(B3 - target3)
    # Balance coefficient fidelity while slightly emphasizing A1/A2 because they drive the low-order host.
    return float((1.5 * err1) + (1.25 * err2) + (1.0 * err3))


def _random_eigs(rng: np.random.Generator, *, radius_max: float) -> np.ndarray:
    radii = radius_max * np.sqrt(rng.random(3, dtype=float))
...
def _fit_normal3(
    *,
    ...
    # Seed with clipped exact roots as one candidate.
    roots = np.roots([1.0, float(A1), float(A2), float(A3)]).astype(np.complex128)
    if roots.size == 3:
        clipped = np.array(
            [
                complex(min(abs(z), float(radius_max)) * np.exp(1j * np.angle(z)))
                for z in roots.tolist()
            ],
            dtype=np.complex128,
        )
        obj = _objective(clipped, A1=float(A1), A2=float(A2), A3=float(A3))
        best_obj = float(obj)
        best_eigs = np.asarray(clipped, dtype=np.complex128)

    for _ in range(int(n_random)):
        cand = _random_eigs(rng, radius_max=float(radius_max))
        obj = _objective(cand, A1=float(A1), A2=float(A2), A3=float(A3))
        if obj < best_obj:
            best_obj = float(obj)
            best_eigs = np.asarray(cand, dtype=np.complex128)

    if best_eigs is None:
        raise RuntimeError("failed to initialize normal3 search")
    ...
    B1, B2, B3 = _coeffs_from_eigs(best_eigs)
    meta = {
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Fit a more flexible normal 3x3 local frontend from corrected A1,A2,A3 by deterministic search over damped eigenvalues, "
    ...
    ap.add_argument("--local_steps", type=int, default=25)
    ...
    ap.add_argument("--out_prefix", default="out/corrected_factor_normal3search_u020")
    args = ap.parse_args()
