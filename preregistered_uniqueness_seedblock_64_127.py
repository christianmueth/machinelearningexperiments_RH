...
def main():
    ...
    ap.add_argument("--out_root", default="out_phase3B_preregistered_uniqueness_64_127")
    ...
    # Preregistered backend set (3 only)
    backends = [
        {"backend_label": "legacy", "tp_backend": "legacy"},
        {
            "backend_label": "geom_v4_theta0p125",
    ...
    if len(big) and "reject" in big.columns:
        by_seed = (
            ...
            .agg(
                ...
                best_dn_gap_min=("dn_gap_min", "max") if "dn_gap_min" in big.columns else ("p_family", lambda x: np.nan),
    ...
    # Tp delta + commutator summaries
    stat_cols = [c for c in ["tp_delta_mean", "tp_delta_max", "tp_comm_mean", "tp_comm_max"] if c in big.columns]
    if len(big) and stat_cols:
        tp_stats = (
            big.groupby(["backend_label"], dropna=False)[stat_cols]
            .agg(["mean", "median", "max"])
            .reset_index()
        )
        # flatten columns
        tp_stats.columns = [
            ...
            for col in tp_stats.columns
        ]
    else:
        tp_stats = pd.DataFrame()
    tp_stats.to_csv(os.path.join(out_root, "tp_stats_summary.csv"), index=False)
    ...
    # Minimal stage-gate table (scan)
    stage_gate = frac.copy() if len(frac) else pd.DataFrame()
    stage_gate.to_csv(os.path.join(out_root, "stage_gate_table.csv"), index=False)

    print("\n=== DONE preregistered uniqueness seedblock ===")
