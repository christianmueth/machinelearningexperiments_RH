...
def main():
    ...
    ap.add_argument("--out_root", default="out_phase3B_confirm_preregistered_rejecting_seeds_N16384")
    ap.add_argument("--seeds", default="36,42,45,54,58")
    ...
    ap.add_argument("--resume", action="store_true", help="Skip jobs whose out_dir already contains rows.csv")
    args = ap.parse_args()
    ...
    if len(big) and "reject" in big.columns:
        agg = (
            ...
            .agg(
                ...
                median_dn_delta=("dn_delta", "median") if "dn_delta" in big.columns else ("p_family", lambda x: float("nan")),
            )
            .reset_index()
