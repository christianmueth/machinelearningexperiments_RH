...
def main():
    ...
    ap.add_argument("--out_root", default="out_phase3B_preregistered_uniqueness_64_127_v9_phasetransport")
    ...
    # v9 knobs (kept as args so you can prereg different constants without editing code)
    ...
    # Preregistered backend set (2 only): legacy + v9-autotune
    backends = [
    ...
    if len(big) and "reject" in big.columns and pcol is not None:
        by_seed = (
            ...
            .agg(
                ...
                median_dn_identity_rank=(
                    "dn_identity_rank",
                    "median",
                )
                if "dn_identity_rank" in big.columns
                else (pcol, lambda x: np.nan),
                rows=("reject", "size"),
            )
            .reset_index()
        )
    ...
    uniq_cols = [
        ...
        "tp_comm_max",
        "geom_v9_eps_global",
        "geom_v9_eps_global_auto",
        "geom_v9_transport_rule",
        "geom_v9_transport_lambda",
        "geom_v9_transport_clip",
        "v9_theta_tr_digest",
        "eps_auto_tune",
        "eps_target_delta",
    ]
    ...
    print(f"DONE prereg v9-scan: rows={len(big)} elapsed={time.time()-t0:.1f}s out_root={out_root}")
